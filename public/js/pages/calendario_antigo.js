...
import { listContractsForRange } from "../app.db.fallback.js"; // adição compatível (novo arquivo)
...
const WEEKDAYS = {
  ...
  "sab":6,"sábado":6,"sat":6,"saturday":6
};
...
async function loadFilters(){
  try {
    if (els.room) {
      ...
      els.room.innerHTML = '<option value="">(todas)</option>' + rooms.map(r => `<option value="${r.id}">${r.name||r.id}</option>`).join('');
    }
    if (els.prof) {
      const pros = await listProfessionals();
      els.prof.innerHTML = '<option value="">(todos)</option>' + pros.map(p => `<option value="${p.id}">${p.name||p.id}</option>`).join('');
    }
  } catch (e) {
    console.error(e);
  }
  ...
  try{
    ...
    if (!items.length){
      ...
      for (const c of contracts){
        ...
        // filtros (room/prof) opcionais
        if (roomId && c.roomId !== roomId) continue;
        if (professionalId && c.professionalId !== professionalId) continue;

        const weekly = Array.isArray(c.weeklySchedule) ? c.weeklySchedule : [];
        for (const dayCfg of weekly){
          const wd = toWeekdayIndex(dayCfg.weekday ?? dayCfg.day ?? dayCfg.dia);
          if (wd === null) continue;

          const dateOfWeek = addDays(start, wd); // segunda + wd
          const dateISO = fmtISO(dateOfWeek);
          if (!within(dateISO, cStartISO, cEndISO)) continue;
          ...
          for (const r of ranges){
            ...
            if (e > s) paint.push({ start: s, end: e });
          }
        }
      }
      items = paint;
...
(async function init(){
  ...
  if (els.week) els.week.value = fmtISO(base);

  await loadFilters();
  await loadAndRender(base);
